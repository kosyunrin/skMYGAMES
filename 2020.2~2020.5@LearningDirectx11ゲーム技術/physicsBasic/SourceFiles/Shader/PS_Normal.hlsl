...
//Directional Light
float4 GetDirectionalIntensity(PS_INPUT input, float4 textureDiffuse, float3 normal);
//Point Light
float4 GetPointIntensity(PS_INPUT input, float4 textureDiffuse, float3 normal);
//Spot Light
float4 GetSpotIntensity(PS_INPUT input, float4 textureDiffuse, float3 normal);
...
float4 main(PS_INPUT input) : SV_TARGET
{
    ...
    float4 DL = GetDirectionalIntensity(input, textureDiffuse, BumpNormal);
    float4 PL = GetPointIntensity(input, textureDiffuse, BumpNormal);
    float4 SL = GetSpotIntensity(input, textureDiffuse, BumpNormal);
...
float4 GetDirectionalIntensity(PS_INPUT input, float4 textureDiffuse, float3 normal)
{
    ...
    float intensity = saturate(dot(-directionLights.Direction.xyz, normal));
...
float4 GetSpotIntensity(PS_INPUT input, float4 textureDiffuse, float3 normal)
{
    ...
    for (int index = 0; index < SLMAX; index++)
    {
        ...
        float4 diffuseColor = spotLights[index].Diffuse * spotLights[index].DiffuseStrength * saturate(dot(normalize(LightToPixel), normal));
