...
public:
	...
	static XMFLOAT3  Vec3Lerp( XMVECTOR from,XMVECTOR to,float t )
	{
		XMFLOAT3 pos;
		XMStoreFloat3(&pos, (1 - t) * from + t * to);
		return pos;
	}
	static XMFLOAT3  Vec3LerpSmooth(XMFLOAT3 from, XMFLOAT3 to, float t,int &out )
	{
		float dis = Tool::Distance_float(from, to);
		...
		XMStoreFloat3(&pos, ((dis - t) * vfrom)/dis + t * vto/dis);
		return pos;
	}
