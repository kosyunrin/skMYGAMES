...
{
	if(!CInput::GetKeyPress(VK_LSHIFT)) SDate.PhysicalBar < 294.8f ? SDate.PhysicalBar += dt * 20 : SDate.PhysicalBar = 294.8f;
...
//	SetCursorPos(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2);
	...
	if (g_Camera != nullptr)
	{
		...
		if (CInput::GetKeyPress('W')||GamePad::ThumbLY()> 0.75f)
		{
			...
			MoveRevise(PlayerPos, ForwardMovemengt, dt,nowface);
		}
	...
	if (pE != nullptr)
	{
		if (ZhunXSwitch)
		{
			...
			if (ray.Hit(pE->GetBodySphereBox()))
			{

				ZhunxColor = 1.0f;
				//pE->Get_Date()->IsHpShow = true;
			}
			else
			{
				ZhunxColor = 0.5f;
				//pE->Get_Date()->IsHpShow = false;
			}
		}
		else
		{
...
void CP0::MoveRevise(XMVECTOR & PlayerPos, XMVECTOR & ForwardMovement,float dt,XMVECTOR nowFaceDir)
{
	CRenderer::Debug_move_v2[0] = 500;
	ForwardMovement = XMVector3Normalize(ForwardMovement);
	nowFaceDir = XMVector3Normalize(nowFaceDir);
	XMFLOAT3 Dir;
	XMStoreFloat3(&Dir, nowFaceDir);
	...
	saigo.Radius = 20.0f;
			 ...
			 CRenderer::Debug_move_v2[0] = 5000;
			 }
			...
			else
			 {
				 ...
				 if (SDate.HP > 3.0f)
				 {
					 if (CInput::GetKeyPress(VK_LSHIFT)&& SDate.PhysicalBar!=0)
					 {
						 Speed = 1.5f;
						 AnimationSpeed = 1.5f;
						 SDate.PhysicalBar > 0.0f ? SDate.PhysicalBar -= dt * 50 : SDate.PhysicalBar = 0.0f;
					 }
					 PlayerPos += ForwardMovement * dt * Speed * P0SP;
				 }
				 else  PlayerPos += ForwardMovement * dt * Speed * P0SPSPEEDDOWN;
			 }
