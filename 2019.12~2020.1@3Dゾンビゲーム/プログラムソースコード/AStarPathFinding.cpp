...
void AStarPathFinding::UpdateX()
{
	...
	if (!isfindEnd)
	{
		...
		for (int i = 0; i < 8; i++)
		{
			...
			if (canwalk(pChild->Pos, pathMap))
			{
				...
				pTemp->child.push_back(pChild);
				pChild->pParent = pTemp;
				//asd.dire = pathMap[pChild->Pos.row][asd.Pos.cow].Center;
				buffer.push_back(pChild);
			}
		}
		//找出F值最小的那个 并从buff中删除这个点
	...
	else
	{
		...
		//if (FindPathDir)
		//{
		//	float Dis = Tool::Distance_float(PathPos[PathCount+1], PathPos[PathCount]);
		//	//rot
		//	XMVECTOR xpos = XMLoadFloat3(&PathDir[PathCount]) * Dis* blend + XMLoadFloat3( &PathPos[PathCount]);
		//	XMFLOAT3 xfpos;
		//	XMStoreFloat3(&xfpos, xpos);
		//	Pos =  xfpos;
		//	if (blend == 1)
		//	{
		//		if (PathCount == PathDir.size() - 1) return;
		//		PathCount++;
		//		blend = 0;
		//	}
		//}
	}
	CRenderer::Debug_show_int[0] = PathDir.size();
}

void AStarPathFinding::UPdateObject(XMFLOAT3& Pos,bool& stopmove,bool& Again,float Speed)
{
	if (FindPathDir)
	{
		
		int blendx = 0;
		Pos = Tool::Vec3LerpSmooth(PathPos[PathCount], PathPos[PathCount + 1], Speed, blendx);
		if (blendx == 1)
		{
			if (PathCount == PathDir.size() - 1)
			{
				Again = true;
				return;
			}
...
void AStarPathFinding::GetPathDir()
{
	int Allsize = VPath.size();
	int Count = 0;
	XMFLOAT3 LastDir;
	...
	if (Allsize >= 1)
	{
		for (int i = Allsize - 1; i >= 0; i--)
		{
			if (Count > 0)
			{
				XMFLOAT3 lastpos = VPath[i + 1].dire;
				XMFLOAT3 currentpos = VPath[i].dire;
				XMVECTOR Dir = XMVector4Normalize(XMLoadFloat3(&currentpos) - XMLoadFloat3(&lastpos));
				if (Count > 1)
				{
					float dot = XMVectorGetY(XMVector3Cross(XMLoadFloat3(& LastDir), Dir));
					if (dot != 0)
					{
						PathPos.push_back(lastpos);
						XMFLOAT3 dir;
	...
	PathPos.push_back(VPath[0].dire);
}
