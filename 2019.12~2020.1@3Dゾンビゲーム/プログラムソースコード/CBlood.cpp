...
void CBlood::Draw()
{
	if (!usemWorldMatrix)
	{
		...
		w0 *= XMLoadFloat4x4(&OtherMatrix);
		//w0 *= XMMatrixScaling(m_Scale.x, m_Scale.y, m_Scale.z);

		XMStoreFloat4x4(&world1[0], w0);

		XMMATRIX w1;
		w1 = XMMatrixRotationRollPitchYaw(XMConvertToRadians(-90.0f), 0.0f, 0.0f);
		//w1 *= XMMatrixScaling(m_Scale.x, m_Scale.y, m_Scale.z);
...
void CBlood::OpenOther(XMMATRIX Matrix)
{
	usemWorldMatrix = false;
	XMStoreFloat4x4(&OtherMatrix, Matrix);
}
