...
void UImanager3D::Init()
{
	...
	Balck = std::make_unique<CPolygon>();
	Balck->Init2D(1);

	BossLife = std::make_unique<CPolygon>();
	BossLife->Init2D(1);
...
void UImanager3D::Draw()
{
	...
	Sm->PNGDraw2D(0, 0, 250, 250, 17, 1.0f);
	CP0* pP0 = CManager::GetScene()->GetGmeobjs<CP0>(4);
	CEnemy* pE=CManager::GetScene()->GetGmeobjs<CEnemy>(6);
	CCameraOne* Cam_p = CManager::GetScene()->GetGmeobjs<CCameraOne>(_Camera);
	...
	XMMATRIX Balckx;
	Balckx = XMMatrixRotationRollPitchYaw(1.6, 0.0f, 0.0f);
	Balckx *= XMMatrixScaling(1.0f, 1.0f, 1.0f);
	Balckx *= XMMatrixTranslation(-50, 100, 440);
	...
	Balck->DrawPlayerLife(0.0f, 0.0f, 0.0f, 120, 120, 1.0f, 1.0f, 1.0f, 1.0f, 1, Balckx);
	
	ZOMBIEDATE* pEdata = pE->Get_Date();
	if (pEdata->IsHpShow)
	{
		float angle = atan2(pE->GetPosition().x - Cam_p->GetPosition().x, pE->GetPosition().z - Cam_p->GetPosition().z);
		XMMATRIX boss;
		boss = XMMatrixRotationRollPitchYaw(1.5f, angle, 0.0f);
		//boss *= XMMatrixScaling(1.0f, 1.0f, 1.0f);
		boss *= pE->XMGet_WorldMatrix();
		boss *= XMMatrixTranslation(1, 2, 1);

		BossLife->DrawPlayerLife(-50, 10.0f, -50, pEdata->HP, 3, 1.0f, 1.0f, 1.0f, 1.0f, 1, boss);
	}
	//2D
	if (SceneSwtich)
	{
