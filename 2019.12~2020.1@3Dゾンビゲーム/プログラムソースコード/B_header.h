...
public:
	static void AttackManger(Ray ray)
	{
		...
		if (ray.Hit(pE->GetEyeSphereBox()))
		{
			if (!pE->Get_Date()->StopChangeState)
			pE->MoveHurtx();
			XMFLOAT3 BloodPos = pE->GetpWeakPos();
			CBlood* pBlood = CManager::GetScene()->AddGameObject<CBlood>(10);



			pE->Get_Date()->HpShowTimer = 0;
			pE->Get_Date()->IsHpShow = true;
			pE->Get_Date()->HP -= 1.0f;
			pE->Get_Date()->xHp += 1.0f;
			if (pE->Get_Date()->HP < 0)
			{
				pE->Get_Date()->HP = 0;
			}
			...
			pBlood->SetScale({ 3,3,3 });
			pBlood->OpenOther(pE->GetMatrixForWorld());
		}
