...
void SkinnedModel::Load(std::string path)
{
	...
	scene = importer.ReadFile(modelpath, aiProcess_Triangulate | aiProcess_CalcTangentSpace | aiProcess_FlipUVs);
	if (scene == nullptr)
	{
	...
	GlobalInverse = XMMATRIX(&scene->mRootNode->mTransformation.a1);
	GlobalInverse = XMMatrixInverse(&determinant, GlobalInverse);

	this->ProcessNode(scene->mRootNode, XMMatrixIdentity());
...
void SkinnedModel::Update(bool updateCollision)
{
	animTime1 += 0.02f;
	animTime2 += 0.02f;
...
void SkinnedModel::RestrictTextures(std::string* texPath, UINT texCount, UINT meshID)
{
...
void SkinnedModel::ReduceTimer(float v)
{
	animTime1 -= v;
...
void SkinnedModel::Render(XMMATRIX worldMatrix, UINT animation, UINT duration)
{
	...
	if (currentAnimation != animation)
	{
		...
		lastAnimationDuration = currentAnimationDuration;
...
void SkinnedModel::Render(std::vector<Collision*> collisions, XMMATRIX worldMatrix, UINT animation, UINT duration)
{
	Shader::SetVSShader(VSShaderID::VS_Bones);
...
void SkinnedModel::ProcessNode(aiNode* node, const XMMATRIX parentMatrix)
{
...
SkinnedMesh SkinnedModel::ProcessMesh(aiMesh* mesh, XMMATRIX transformMatrix)
{
	SkinnedMesh m;
...
std::vector<Texture> SkinnedModel::ProcessMaterialTextures(aiMaterial* material)
{
...
//	XMMATRIX parentMatrix = XMMatrixIdentity();
	...
	parentMatrix *= XMMatrixRotationY(XMConvertToRadians(180.0f));

	//Animation Blend
	if (blendTime > 0.0f)
	{
		float ticksPerSecond = scene->mAnimations[currentAnimation]->mTicksPerSecond != 0 ?
			scene->mAnimations[currentAnimation]->mTicksPerSecond : ticksPerSecond = 25.0f;
		float timeInTicks = animTime1 * ticksPerSecond;
		float animationTime1 = fmod(timeInTicks, currentAnimationDuration);

		ticksPerSecond = scene->mAnimations[lastAnimation]->mTicksPerSecond != 0 ?
			scene->mAnimations[lastAnimation]->mTicksPerSecond : ticksPerSecond = 25.0f;
		timeInTicks = animTime2 * ticksPerSecond;
		float animationTime2 = fmod(timeInTicks, lastAnimationDuration);

		ReadNodeHeirarchy(scene->mRootNode, scene->mRootNode, parentMatrix, animationTime1, animationTime2, currentAnimation, lastAnimation);
	}
	else
	{
		float ticksPerSecond = scene->mAnimations[currentAnimation]->mTicksPerSecond != 0 ?
			scene->mAnimations[currentAnimation]->mTicksPerSecond : ticksPerSecond = 25.0f;
		float timeInTicks = animTime1 * ticksPerSecond;
		float animationTime = fmod(timeInTicks, currentAnimationDuration);

		ReadNodeHeirarchy(scene->mRootNode, parentMatrix, animationTime, currentAnimation);
	}
...
void SkinnedModel::ReadNodeHeirarchy(const aiNode* node, const XMMATRIX parentTransform, float time, UINT action)
{
	...
	const aiAnimation* animation = scene->mAnimations[action];
...
void SkinnedModel::ReadNodeHeirarchy(const aiNode* node1, const aiNode* node2, const XMMATRIX parentTransform, float time1, float time2, UINT action1, UINT action2)
{
	...
	const aiAnimation* animation1 = scene->mAnimations[action1];
	const aiAnimation* animation2 = scene->mAnimations[action2];
	...
	XMMATRIX globalTransform = parentTransform * nodeTransform1;
